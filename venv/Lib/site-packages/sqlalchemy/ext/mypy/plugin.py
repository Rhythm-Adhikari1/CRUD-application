...
class CustomPlugin(Plugin):
    ...
    def get_class_decorator_hook(
        ...
        if (
            sym is not None
            and names._type_id_for_named_node(sym.node) is names.MAPPED_DECORATOR
        ):
...
def _fill_in_decorators(ctx: ClassDefContext) -> None:
    for decorator in ctx.cls.decorators:
        ...
        # skipped.
        if isinstance(decorator, nodes.MemberExpr) and decorator.name == "mapped":

            sym = ctx.api.lookup(decorator.expr.name, decorator, suppress_errors=True)
            if sym:
