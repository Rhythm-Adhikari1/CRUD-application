...
class DeclClassApplied:
    ...
    def deserialize(
        ...
        return DeclClassApplied(
            ...
            mapped_mro=[
                deserialize_and_fixup_type(type_, api) for type_ in data["mapped_mro"]
            ],
        )
...
def _infer_type_from_relationship(
    ...
    collection_cls_arg = util._get_callexpr_kwarg(stmt.rvalue, "collection_class")
...
def _infer_type_from_decl_column(
    ...
    if names._mro_has_id(callee.node.mro, names.TYPEENGINE):
        python_type_for_type = _extract_python_type_from_typeengine(callee.node)
...
def _extract_python_type_from_typeengine(node: TypeInfo) -> Instance:
    for mr in node.mro:
        if (
            mr.bases
            and mr.bases[-1].type.fullname == "sqlalchemy.sql.type_api.TypeEngine"
        ):
